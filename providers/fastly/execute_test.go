...
func TestFastlyExecuteRetainsPartialResults(t *testing.T) {
...
func TestFastlyExecuteCancellationDuringSubmission(t *testing.T) {
	ctx, cancel := context.WithCancel(t.Context())
	defer cancel()
	var calls atomic.Int32
	transport := &http.Transport{DialContext: func(context.Context, string, string) (net.Conn, error) {
...
func TestFastlyExecuteBoundsAndCompletesResponseReading(t *testing.T) {
	const limit = 64 * 1024
	base := `{"status":"ok"}`
	tests := []struct {
		name          string
...
func TestFastlyExecuteDoesNotFollowRedirects(t *testing.T) {
